...
async def get_jwks() -> dict:
    ...
    async with _jwks_lock:
        ...
        async with httpx.AsyncClient(timeout=10.0, verify=settings.keycloak_tls_verify) as client:
            response = await client.get(settings.keycloak_jwks_url)
            response.raise_for_status()
            keys = response.json()

        _jwks_cache = (keys, time.monotonic())
        logger.info("jwks_refreshed", url=settings.keycloak_jwks_url)
        return keys
...
async def verify_token(
    ...
    try:
        # Audience verification is skipped: Keycloak access tokens issued for the
        # frontend client carry aud=["account"] by default (no audience mapper configured).
